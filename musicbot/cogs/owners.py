...
class Owners (commands.Cog) :
    ...
    async def serverinfo(self, ctx) :
        ...
        embed.add_field(name="플랫폼", value=platform.platform(), inline=False)
        embed.add_field(name="커널", value=platform.version(), inline=False)
        embed.add_field(name="아키텍쳐", value=platform.machine(), inline=False)
        embed.add_field(name="CPU 사용량", value=str(psutil.cpu_percent()) +"%", inline=False)
        memorystr = str(round((psutil.virtual_memory().used / (1024.0 ** 3)), 1)) + "GB" + " / " + str(round((psutil.virtual_memory().total / (1024.0 ** 3)), 1)) + "GB"
        embed.add_field(name="Memory(램) 사용량", value=memorystr, inline=False)
        embed.add_field(name="Python 버전", value=("%s %s") %(platform.python_implementation(), platform.python_version()), inline=False)
        embed.add_field(name="Discord.py 버전", value=discord.__version__, inline=False)
        embed.add_field(name="Lavalink.py 버전", value=lavalink.__version__, inline=False)
        embed.add_field(name="핑", value=str(round(self.bot.latency * 1000)) + "ms", inline=False)
        embed.set_footer(text=BOT_NAME_TAG_VER)
        await ctx.send(embed=embed)
