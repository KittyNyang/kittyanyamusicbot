...
class Music(commands.Cog):
    ...
    @commands.command(aliases=['join', '들어와', 'c', 'ㅊ', '연결', '이리와'])
    ...
    async def play(self, ctx, *, query: str = None):
        ...
        if results['loadType'] == 'PLAYLIST_LOADED':
            ...
            player.add(requester=ctx.author.id, track=track)
        embed.set_thumbnail(url="http://img.youtube.com/vi/%s/maxresdefault.jpg" %(info['identifier']))
        embed.set_footer(text=BOT_NAME_TAG_VER)
    ...
    @commands.command(aliases=['빌보드재생ㅣㅏ', '빌보드차트재생ㅣㅏ', '빌보드음악ㅣㅏ', 'ㅂㅈㅣㅏ', 'qwㅣㅏ'])
    ...
    @commands.command(aliases=['빌보드재팬재생ㅣㅏ', '빌보드차트재팬재생ㅏㅣ', '빌보드재팬음악ㅣㅏ', 'ㅂㅈㅈㅣㅏ', 'qwwㅣㅏ'])
    ...
    @commands.command(aliases=['np', 'n', 'playing', '현재재생중', 'ㅜ', 'ㅞ', 'ㅜㅔ', '현플'])
    async def now(self, ctx):
        ...
        song = f'곡제목: **[{player.current.title}]({player.current.uri})**\n재생중: {position}  |  곡 길이: {duration}'
        embed = discord.Embed(color=self.normal_color,
                              title=get_lan(ctx.author.id, "music_now_playing"), description=song)
        embed.set_image(url="%s/maxresdefault.jpg"%player.current.uri.replace('https://www.youtube.com/watch?v=', 'http://img.youtube.com/vi/'))
        embed.set_footer(text=BOT_NAME_TAG_VER)
        await ctx.send(embed=embed)
    ...
    @commands.command(aliases=['resume', '일시정지', '일시중지', '재개', '다시재생'])
    ...
    @commands.command(aliases=['셔플', '섞기'])
    ...
    @commands.command(aliases=['loop', 'l', '반복', 'ㅣ', '루프'])
    ...
    @commands.command(aliases=['제거', 'rm', '삭제'])
    ...
    @commands.command(aliases=['dc', '연결해제', '나가', 'ㅇㅊ', '중지', '정지', 'stop', '저리가'])
