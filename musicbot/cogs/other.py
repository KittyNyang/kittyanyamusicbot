...
class Other (commands.Cog) :
    ...
    async def invite(self, ctx):
        link = 'https://discord.com/oauth2/authorize?client_id=%s&permissions=8&scope=bot' %self.bot.user.id
    ...
    async def softver(self, ctx) :
        ...
        embed.add_field(name="Java 버전", value=f"```{javaver}```", inline=False)
        embed.add_field(name="Lavalink 버전", value=f"```{lavalinkver}```", inline=False)
        embed.set_footer(text=BOT_NAME_TAG_VER)
        await ctx.send(embed=embed)
